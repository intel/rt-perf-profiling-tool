...
#define P_IRQ       9   /* add by hkh */
#define P_NUMBER	10	/* one more than the max */
...
#define LSCPU_STRLEN 512
	...
	while (fgets(tmpstr, LSCPU_STRLEN, pop) != NULL) {
	    ...
	    if (strncmp("Byte Order:", tmpstr, strlen("Byte Order:")) == 0) {
			if (lscpu.byte_order != NULL)
			{
			    FREE(lscpu.byte_order);
				lscpu.byte_order = NULL;
			}
	    ...
	    if (strncmp
			...
			if (lscpu.cpu_online != NULL)
			{
				FREE(lscpu.cpu_online);
				lscpu.cpu_online = NULL;
			}
	    ...
	    if (strncmp
			...
			if (lscpu.cpu_offline != NULL)
			{
			    FREE(lscpu.cpu_offline);
				lscpu.cpu_offline = NULL;
			}
			lscpu.cpu_offline =
	    ...
	    if (strncmp("Model name:", tmpstr, strlen("Model name:")) == 0) {
			if (lscpu.model_name != NULL)
			{
				FREE(lscpu.model_name);
				lscpu.model_name = NULL;
			}
...
#define SHOW_SHM_NONE	0
#define SHOW_SHM_ALL	1
#define SHOW_SHM_CLEAR	2

int show_shm = 0;
int sharemem_init_flag = 0;
...
struct{
	int64_t period_ns;
	float max_cycle_1sec;
	float max_jitter_1sec;
	unsigned int cycle_counter;
	int circle_x;
	int circle_y;
	float min_cycle_time;
	float max_cycle_time;
	float min_jitter_time;
	float max_jitter_time;
	int cycle_counter_max_cycle;
	int cycle_counter_max_jitter;
	float receive_ns;
	float process_ns;
	float send_ns;
	float motor_ns;
...
int read_buf_int32(char *buf)
{
	unsigned int result = 0;
	result += (unsigned int)((unsigned char)buf[3]) << 8;
	result += (unsigned int)((unsigned char)buf[2]) << 0;
	result += (unsigned int)((unsigned char)buf[1]) << 24;
	result += (unsigned int)((unsigned char)buf[0]) << 16;
	return (int)result;
}

unsigned int read_buf_uint32(char* buf)
{
	unsigned int result = 0;
	result += (unsigned int)((unsigned char)buf[3]) << 8;
	result += (unsigned int)((unsigned char)buf[2]) << 0;
...
int64_t read_buf_int64(char* buf)
{
	uint64_t result = 0;
	result += (uint64_t)((unsigned char)buf[7]) << 8;
	result += (uint64_t)((unsigned char)buf[6]) << 0;
	result += (uint64_t)((unsigned char)buf[5]) << 24;
	result += (uint64_t)((unsigned char)buf[4]) << 16;
	result += (uint64_t)((unsigned char)buf[3]) << 40;
	result += (uint64_t)((unsigned char)buf[2]) << 32;
	result += (uint64_t)((unsigned char)buf[1]) << 56;
	result += (uint64_t)((unsigned char)buf[0]) << 48;
...
{
	if(show_shm){
		sharemem.size = 0x1000;
		sharemem.status = 0;
		sharemem.id = shmget((key_t)87654321,sharemem.size,IPC_CREAT|0666);
		printf("shmid: %u\n",sharemem.id);
		if(sharemem.id == -1){
		...
		sharemem.addr = shmat(sharemem.id,NULL,0);
		if(sharemem.addr == (void *)-1){
			printf("share memory get addr error \n");
			sharemem.status = 2;
		}
		memset(sharemem.buff,0,sharemem.size);
		sharemem_init_flag = 1;
	}
}
...
{
	if(show_shm){
		if(shmdt(sharemem.addr) == -1){
			printf("share memory exit error \n");
			exit(1);
		}
	}
}
...
void proc_init()
{
    ...
    proc[P_IRQ].filename = "/proc/interrupts";
}
...
struct irq_node {
    char name[4];
    char msg[128];
    unsigned long cpu[256];
};

struct irq_stat {
    unsigned char num;
    struct irq_node irq_msg[100];
	unsigned long count[256];
};
struct irq_stat irq_pre;
...
/* Only try to read /proc/ppc64/lparcfg once - remember if it's readable */
    ...
    str = locate("lparcfg");
    sscanf(str, "lparcfg %16s", lparcfg.version_string);
    str = locate("serial_number");
    sscanf(str, "serial_number=%16s", lparcfg.serial_number);
    str = locate("system_type");
    for (i = 0; i < strlen(str); i++)	/* Remove new spaces in massive string meaning PowerKVM Guest !! */
	if (str[i] == ' ')
	    str[i] = '-';
    sscanf(str, "system_type=%64s", lparcfg.system_type);
    if (power_vm_type == VM_UNKNOWN) {
...
int show_irq = 0;
unsigned long char2long(char* buf,int num)
{
    ...
    for(i=0; i<num; i++){
        if(buf[i] == 0x20)
            continue;
        result += (buf[i]-0x30)*pow(10,num-i-1);
    }
    return result;
}
...
{
    int i,j,len,start;
    p->irq.num = proc[P_IRQ].lines - 1;
	for(j=0;j<cpus;j++)
		p->irq.count[j] = 0;
    for(i=1;i<proc[P_IRQ].lines;i++){
        len = strlen(proc[P_IRQ].line[i]);
		if(proc[P_IRQ].line[i][4] == ':'){
        	memmove(p->irq.irq_msg[i-1].name, &proc[P_IRQ].line[i][1], 3);
			len -= 5;
			start = 5;
		}else{
        	memmove(p->irq.irq_msg[i-1].name, &proc[P_IRQ].line[i][0], 3);
        	len -= 4;
			start = 4;
		}
        p->irq.irq_msg[i-1].name[3] = '\0';
        for(j=0;j<cpus;j++){
            p->irq.irq_msg[i-1].cpu[j] = char2long(&proc[P_IRQ].line[i][start+j*11],11);
            p->irq.count[j] += char2long(&proc[P_IRQ].line[i][start+j*11],11);
            len -= 11;
            if(len < 11)
                break;
        }
        if(j == cpus)
            strcpy(p->irq.irq_msg[i-1].msg, &proc[P_IRQ].line[i][start+cpus*11+3]);
        else
            memset(p->irq.irq_msg[i-1].msg, '\0', 128);
    }
}
...
void proc_disk_io(double elapsed)
{
	...
	if (str == 0)
	    break;
	ret = sscanf(str, " (%d,%d):(%ld,%lu,%lu,%lu,%lu",
		     &p->dk[i].dk_major,
		     &p->dk[i].dk_minor,
		     &p->dk[i].dk_noinfo,
		     &p->dk[i].dk_reads,
...
*/
	...
	ret =
	    sscanf(&buf[0],
		   "%d %d %32s %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu",
...
*/
	...
	ret =
	    sscanf(&buf[0],
		   "%d %d %lu %32s %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu",
		   &p->dk[i].dk_major, &p->dk[i].dk_minor,
	      ...
	      double kernel, double iowait, double idle, double steal)
{
	...
	if (cpu_no == 0)
	    fprintf(fp, "CPU_ALL,%s,%.1lf,%.1lf,%.1lf,%.1f,%.1lf,,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
		    LOOP, user, kernel, iowait, idle, steal, cpus, rt.min_cycle_time,rt.max_cycle_time,rt.min_jitter_time,rt.max_jitter_time,rt.max_cycle_1sec,rt.max_jitter_1sec);
	else {
...
char *save_word(char *in, char *out, int size)
{
    int len;
    int i;
    len = strlen(in);
...
void load_dgroup(struct dsk_stat *dk)
{
	...
	/* save the name */
	nextp = save_word(line, name, 1024);
	...
	for (i = 0; i < DGROUPITEMS && *nextp != 0; i++) {
	    nextp = save_word(nextp, name, 1024);
	    for (j = 0; j < disks; j++) {
...
void hint(void)
{
	...
	printf
	("\t-e            Includes RT Performance stats\n");
	printf
	("\t-i            Includes Interrupt stats\n");
    printf
...
void help(void)
{
	...
	printf
	("\t-e            Ethercat master RT performance in share memory\n");
    ...
    printf
	("\t-i            Get system interrupt infomation \n");
    printf
    ...
    printf("\ti   = Open or close interrupt information\n");
...
/* alphabetic order from here */
		...
		case 'd':
		    ...
		    break;
		case 'e': //add by hkh
			show_shm = SHOW_SHM_ALL;
			if(!sharemem_init_flag)
				sharemem_init();
			wclear(padshm);
			break;
		case 'E':
			show_shm = SHOW_SHM_CLEAR;
			wclear(padshm);
			break;
		case 'G':
		    if (auto_dgroup) {
...
*/
	...
	ret =
	    sscanf(&buf[0],
		   "%17s %llu %llu %lu %lu %lu %lu %lu %lu %llu %llu %lu %lu %lu %lu %lu",
...
static void get_options(int argc, char **argv)
{
    ...
    while (-1 !=
	   (i =
	    getopt(argc, argv,
		   "?abc:C:Dd:eEifF:g:hI:Jl:m:MNpr:Rs:tTUVxXz"))) {
	switch (i) {
	case '?':
	    hint();
	    exit(0);
	...
	case 'F':		/* background mode with user supplied filename */
		len = strlen(optarg);
		if(!user_filename)
			user_filename = MALLOC(len + 1);
		if(user_filename == NULL)
			exit(-1);
		else{
			strncpy(user_filename, optarg, len);
			user_filename[len] = '\0';
			user_filename_set++;
...
static int main_loop(int argc, char **argv, int max_define_loop)
{
    int secs;
    int cpu_idle;
    int cpu_user;
...
/* External Data Collector Controls */
	...
	padshm = newpad(10, MAXCOLS);
	padirq = newpad(40, MAXCOLS);
	...
	if(open_dirname == NULL){
        open_dirname = MALLOC(64);
        snprintf(open_dirname, 63, "%02d%02d%02d_%02d%02d/",tim->tm_year,tim->tm_mon, tim->tm_mday, tim->tm_hour, tim->tm_min);
        if(access(open_dirname, F_OK) != 0){
            if(mkdir(open_dirname, 0755) == -1){
                perror("mkdir error");
                exit(-1);
            }
        }
	...
	if (varperftmp) {
	...
	else if (user_filename_set && user_filename != NULL) {
	    ...
	    snprintf(open_filename, strlen(hostname) + 63, "%s%s_%02d%02d%02d_%02d%02d.nmon",
			open_dirname,
		    hostname,
		    tim->tm_year,
...
//		using_stdout = 1;
	...
	fflush(NULL);
	fprintf(fp,"RT,RT Total,min_cycle_time,max_cycle_time,min_jitter_time,max_jitter_time,max_cycle_1sec,max_jitter_1sec\n");
	proc_read(P_IRQ);
	proc_interrupt();
	fprintf(fp,"IRQ,Message,");
	#if 0
	for(i=0;i<p->irq.num;i++){
		if(i == (p->irq.num - 1))
			fprintf(fp,"%s:%s",p->irq.irq_msg[i].name, p->irq.irq_msg[i].msg);
		else
			fprintf(fp,"%s:%s,",p->irq.irq_msg[i].name, p->irq.irq_msg[i].msg);
	}
	#endif
    fprintf(fp, "irq_change,min_cycle_time,max_cycle_time,min_jitter_time,max_jitter_time,max_cycle_1sec,max_jitter_1sec");
	fprintf(fp, "\n");
	...
	for (i = 1; i <= cpus; i++)
	    fprintf(fp,
		    ...
		    i, i, run_name);
	fprintf(fp,
		"CPU_ALL,CPU Total %s,User%%,Sys%%,Wait%%,Idle%%,Steal%%,Busy,CPUs,min_cycle_time,max_cycle_time,min_jitter_time,max_jitter_time,max_cycle_1sec,max_jitter_1sec\n",
		run_name);
	if (show_mhz) {
	...
	fprintf(fp,
		"MEM,Memory MB %s,memtotal,hightotal,lowtotal,swaptotal,memfree,highfree,lowfree,swapfree,memshared,cached,active,bigfree,buffers,swapcached,inactive,min_cycle_time,max_cycle_time,min_jitter_time,max_jitter_time,max_cycle_1sec,max_jitter_1sec\n",
		run_name);
	...
	fprintf(fp,
		"PROC,Processes %s,Runnable,Blocked,pswitch,syscall,read,write,fork,exec,sem,msg,min_cycle_time,max_cycle_time,min_jitter_time,max_jitter_time,max_cycle_1sec,max_jitter_1sec\n",
		run_name);
/*
...
*/
	...
	fprintf(fp, ",min_cycle_time,max_cycle_time,min_jitter_time,max_jitter_time,max_cycle_1sec,max_jitter_1sec");
	...
	if (cursed) {		/* Top line */
		...
		mvwprintw(padwelcome, x + 15, 3, "  c = CPU         l = CPU Long-term     - = Faster screen updates     e = Open RT screen");
		mvwprintw(padwelcome, x + 16, 3, "  C = \" WideView  U = Utilisation       + = Slower screen updates     E = Close RT screen");
		mvwprintw(padwelcome, x + 17, 3, "  m = Memory      V = Virtual memory    j = File Systems              i = Interrupt");
	...
	if (show_help && cursed) {
	    ...
	    mvwprintw(padhelp, 13, 1,
		      "e = Open RT screen                    | E = Close RT screen");
	    mvwprintw(padhelp, 14, 39, "| b = black & white mode");
	    mvwprintw(padhelp, 15, 1,
		      "Other Controls:                       | i = interrupt");
...
*/
	...
	if (show_shm == SHOW_SHM_ALL){
		memcpy(sharemem.buff, (pthread_mutex_t *)sharemem.addr, sharemem.size);
		rt.period_ns 				= read_buf_int64(sharemem.buff);
		rt.max_cycle_1sec 		    = read_buf_int64(sharemem.buff+8)/1000.0;
		rt.max_jitter_1sec 		    = read_buf_int64(sharemem.buff+16)/1000.0;
		rt.cycle_counter 			= read_buf_uint32(sharemem.buff+24);
		rt.circle_x 				= read_buf_int32(sharemem.buff+28);
		rt.circle_y 				= read_buf_int32(sharemem.buff+32);
		rt.min_cycle_time 			= read_buf_int64(sharemem.buff+36)/1000.0;
		rt.max_cycle_time 			= read_buf_int64(sharemem.buff+44)/1000.0;
		rt.min_jitter_time 			= read_buf_int64(sharemem.buff+52)/1000.0;
		rt.max_jitter_time 			= read_buf_int64(sharemem.buff+60)/1000.0;
		rt.cycle_counter_max_cycle  = read_buf_int32(sharemem.buff+68);
		rt.cycle_counter_max_jitter = read_buf_int32(sharemem.buff+72);
		rt.receive_ns 				= read_buf_int64(sharemem.buff+76)/1000.0;
		rt.process_ns 				= read_buf_int64(sharemem.buff+84)/1000.0;
		rt.send_ns 					= read_buf_int64(sharemem.buff+92)/1000.0;
		rt.motor_ns 				= read_buf_int64(sharemem.buff+100)/1000.0;
		rt.other_ns 				= read_buf_int64(sharemem.buff+108)/1000.0;
		rt.sharemem_ns 				= read_buf_int64(sharemem.buff+116)/1000.0;

		if(cursed){
			BANNER(padshm, "RT Performance");
		    mvwprintw(padshm, 1, 0, cpu_line);
		    mvwprintw(padshm, 2, 0, "name ");
		    COLOUR wattrset(padshm, COLOR_PAIR(2));	/* Green */
		    mvwprintw(padshm, 2, 10, "cur/us");
		    COLOUR wattrset(padshm, COLOR_PAIR(1));	/* Red */
		    mvwprintw(padshm, 2, 20, "min/us");
		    COLOUR wattrset(padshm, COLOR_PAIR(4));	/* Blue */
		    mvwprintw(padshm, 2, 30, "max/us");

			COLOUR wattrset(padshm, COLOR_PAIR(0));
			mvwprintw(padshm, 3,  0, "cycle ");
			mvwprintw(padshm, 3, 10, "%.3f", rt.max_cycle_1sec);
			mvwprintw(padshm, 3, 20, "%.3f", rt.min_cycle_time);
			mvwprintw(padshm, 3, 30, "%.3f", rt.max_cycle_time);

			mvwprintw(padshm, 4,  0, "jitter ");
			mvwprintw(padshm, 4, 10, "%.3f", rt.max_jitter_1sec);
			mvwprintw(padshm, 4, 20, "%.3f", rt.min_jitter_time);
			mvwprintw(padshm, 4, 30, "%.3f", rt.max_jitter_time);
			mvwprintw(padshm, 5,  0, cpu_line);

			mvwprintw(padshm, 6,  0, "run time:");
			mvwprintw(padshm, 6,  10,"%02dh",rt.cycle_counter/4000/3600);
			mvwprintw(padshm, 6,  13, "%02dm",rt.cycle_counter/4000/60%60);
			mvwprintw(padshm, 6,  16, "%02ds",rt.cycle_counter/4000%60);
			mvwprintw(padshm, 7,  0, cpu_line);

			mvwprintw(padshm, 8,  0, "shmid:");
			mvwprintw(padshm, 8,  6, "%d", sharemem.id);
			mvwprintw(padshm, 9,  0, cpu_line);
			DISPLAY(padshm,10);
		}else{
	        fprintf(fp,"RT,T%04d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
                    loop,
                    rt.min_cycle_time,
                    rt.max_cycle_time,
                    rt.min_jitter_time,
    ...
    if (show_irq){
        ...
        proc_interrupt();
		if(cursed){
			BANNER(padirq, "Interrupt");
		    mvwprintw(padirq, 1, 0, cpu_line);
		    mvwprintw(padirq, 2, 0, "name ");
            for(i=0;i<cpus;i++){
		        COLOUR wattrset(padirq, COLOR_PAIR(i%8));
		        mvwprintw(padirq, 2, 11*(i+1), "CPU%d",i);
            }
			COLOUR wattrset(padirq, COLOR_PAIR(0));
            for(i=0;i<p->irq.num;i++){
			    mvwprintw(padirq, i+3, 0, "%s", p->irq.irq_msg[i].name);
                for(j=0;j<cpus;j++)
			        mvwprintw(padirq, i+3, 11+j*11, "%ld", p->irq.irq_msg[i].cpu[j]);
			    mvwprintw(padirq, i+3, 11+cpus*11, "%s", p->irq.irq_msg[i].msg);
            }
			mvwprintw(padirq, 39,  0, cpu_line);
			DISPLAY(padirq,40);
		}else{
			if (loop == 1){
				for(i=0;i<cpus;i++){
					fprintf(fp,"IRQ_CPU%03d,T%04d,",i+1,loop);
					#if 0
					for(j=0;j<p->irq.num;j++){
						if(j == (p->irq.num-1))
						    fprintf(fp,"0");
						else
							fprintf(fp,"0,");
					...
					#endif
                    fprintf(fp,"0,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f",rt.min_cycle_time,rt.max_cycle_time,rt.min_jitter_time,rt.max_jitter_time,rt.max_cycle_1sec,rt.max_jitter_1sec);
					fprintf(fp,"\n");
				}
			}else{
			    for(i=0;i<cpus;i++){
					...
					for(j=0;j<p->irq.num;j++){
						if(j == (p->irq.num-1))
							fprintf(fp,"%ld",p->irq.irq_msg[j].cpu[i] - irq_pre.irq_msg[j].cpu[i]);
						else
							fprintf(fp,"%ld,",p->irq.irq_msg[j].cpu[i] - irq_pre.irq_msg[j].cpu[i]);
					}
					#endif
					fprintf(fp,"%ld",p->irq.count[i]-irq_pre.count[i]);
                    fprintf(fp,",%.3f,%.3f,%.3f,%.3f,%.3f,%.3f",rt.min_cycle_time,rt.max_cycle_time,rt.min_jitter_time,rt.max_jitter_time,rt.max_cycle_1sec,rt.max_jitter_1sec);
					fprintf(fp,"\n");
				}
			}
			fprintf(fp,"IRQ_CPU_ALL,T%04d,",loop);
			irq_temp = 0;
			for(j=0;j<cpus;j++)
				irq_temp += p->irq.count[j] - irq_pre.count[j];
			if(loop == 1)
				irq_temp = 0;
			fprintf(fp, "%ld",irq_temp);
			#if 0
            for(i=0;i<p->irq.num;i++){
				for(j=0;j<cpus;j++)
					irq_temp += p->irq.irq_msg[i].cpu[j] - irq_pre.irq_msg[i].cpu[j];
				if(loop == 1)
					irq_temp = 0;
				if(i == (p->irq.num-1))
					fprintf(fp,"%ld",irq_temp);
...
/* DEBUG inject steal       cpu_steal = cpu_sys; */
	...
	if (show_util) {
		...
		if (first_util) {
		    fprintf(fp,
			    "CPUUTIL_ALL,CPU Util Stats %s,User%%,Nice%%,Sys%%,Idle%%,Wait%%,Irq%%,Softirq%%,Steal%%,Guest%%,Guest_nice%%,min_cycle_time,max_cycle_time,min_jitter_time,max_jitter_time,max_cycle_1sec,max_jitter_1sec\n",
			    run_name);
		    for (i = 1; i <= cpus; i++)
			fprintf(fp,
		...
		fprintf(fp,
			"CPUUTIL_ALL,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
			...
			RAWTOTAL(steal) / elapsed,
			RAWTOTAL(guest) / elapsed,
			RAWTOTAL(guest_nice) / elapsed,rt.min_cycle_time,rt.max_cycle_time,rt.min_jitter_time,rt.max_jitter_time,rt.max_cycle_1sec,rt.max_jitter_1sec);
		for (i = 0; i < cpus; i++) {
		    fprintf(fp,
...
#define LOWCOL 60
		...
		if (show_rrd)
		    ...
		    str_p =
			"MEM,%s,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n";
		fprintf(fp, str_p, LOOP, p->mem.memtotal / 1024.0,
			...
			p->mem.buffers / 1024.0,
			p->mem.swapcached / 1024.0,
			p->mem.inactive / 1024.0,rt.min_cycle_time,rt.max_cycle_time,rt.min_jitter_time,rt.max_jitter_time,rt.max_cycle_1sec,rt.max_jitter_1sec);
#ifdef POWER
		if (lparcfg.cmo_enabled != 0) {
		    if (!show_rrd)
			fprintf(fp,
...
#define BOOTCOL 55
		...
		if (show_rrd)
		    ...
		    str_p =
			"PROC,%s,%.0f,%.0f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n";
		...
		fprintf(fp, str_p, LOOP, (float) p->cpu_total.running,	/*runqueue */
			...
			-1.0,	/*sem */
			-1.0,
			rt.min_cycle_time,rt.max_cycle_time,rt.min_jitter_time,rt.max_jitter_time,rt.max_cycle_1sec,rt.max_jitter_1sec);	/*msg */
	    }
	}
...
*/
		...
		fprintf(fp, ",%.3f,%.3f,%.3f,%.3f,%.3f,%.3f",
		            rt.min_cycle_time,rt.max_cycle_time,rt.min_jitter_time,rt.max_jitter_time,rt.max_cycle_1sec,rt.max_jitter_1sec);
		fprintf(fp, "\n");
	    }
	}
	errors = 0;
...
#define NUM_CYCLE_PER_FILE	9999
int main(int argc,char** argv)
{
	...
	while(maxloops)
	{
		...
		first_util = 1;
		arg_first_time = 1;
		if(maxloops / NUM_CYCLE_PER_FILE){
			maxloops -= NUM_CYCLE_PER_FILE;
			main_loop(argc,argv,NUM_CYCLE_PER_FILE);
			fclose(fp);
			fp = NULL;
		}else{
			main_loop(argc,argv,maxloops % NUM_CYCLE_PER_FILE);
			maxloops = 0;
		}
	}
	free(user_filename);
